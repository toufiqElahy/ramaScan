defmodule BlockScoutWeb.Tokens.Helpers do
  ...
  defp do_token_transfer_amount(%Token{type: "RAMA-20"}, nil, _token_id) do
    {:ok, "--"}
  end

  defp do_token_transfer_amount(%Token{type: "RAMA-20", decimals: nil}, amount, _token_id) do
    {:ok, CurrencyHelpers.format_according_to_decimals(amount, Decimal.new(0))}
  end

  defp do_token_transfer_amount(%Token{type: "RAMA-20", decimals: decimals}, amount, _token_id) do
    {:ok, CurrencyHelpers.format_according_to_decimals(amount, decimals)}
  end

  defp do_token_transfer_amount(%Token{type: "RAMA-721"}, _amount, _token_id) do
    {:ok, :erc721_instance}
  end
